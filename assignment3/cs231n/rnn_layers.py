...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    dot_x = np.dot(x, Wx)
    dot_h = np.dot(prev_h, Wh)
    logits = dot_x + dot_h + b
    next_h = np.tanh(logits)
    cache = (x, prev_h, b, logits, Wx, Wh)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    (x, prev_h, b, logits, Wx, Wh) = cache

    dtanh = (1 - np.tanh(logits)**2) * dnext_h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.array(h)
    h = np.swapaxes(h, 0, 1)
    cache = (step_caches, x, Wx, Wh, b, h0)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    step_caches, x, Wx, Wh, b, _ = cache
    dh_time = np.swapaxes(dh, 0, 1)
    dprev_h = np.zeros_like(dh_time[0])
    dWx = np.zeros_like(Wx)
    dWh = np.zeros_like(Wh)
    db = np.zeros_like(b)
    list_x = []
    for idx, dh_t in enumerate(reversed(dh_time)):
        step_cache = step_caches[-(idx + 1)]
        ldx, ldprev_h, ldWx, ldWh, ldb = rnn_step_backward(dh_t + dprev_h, step_cache)
        db += ldb
        dWx += ldWx
        dWh += ldWh
        dh0 = ldprev_h
        dprev_h = ldprev_h
        list_x.append(ldx)
    dx = np.array(list(reversed(list_x)))
